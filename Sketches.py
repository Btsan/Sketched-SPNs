...
class FastAGMS(Sketch):
    def __init__(self, data:pd.DataFrame, depth:int, width:int, sign_hashes:list, bin_hashes:list, exact_preds=False, **kwargs):
        ...
        values = self.distincts[self.columns].map(hash).values + 1 # [N, col]
        ...
        #     mask = self.distincts[col].notnull().values[None, :] # [1, N]
        #     self.signs[col] = [sign_hash(values) * mask for sign_hash in sign_hashes]
        #     self.bins[col] = [bin_hash(values) for bin_hash in bin_hashes]
        ...
        if not col_in_keys and not col_in_preds:
            ...
            if col_in_keys:
                ...
                if not self.countmins:
                    ...
                    for col in col_in_preds:
                        # cm = torch.zeros((self.depth * len(self.bin_hashes), self.width), dtype=torch.long)
                        cm = torch.zeros((self.depth, self.width), dtype=torch.bool)
                        if len(selection) > 0:
                            values = selection[col].map(hash).values + 1
                            # bins = torch.concatenate([bin_hash(values) for bin_hash in self.bin_hashes], dim=0) # depth * b, N
                            bins = self.bin_hashes[0](values) # depth, N
                            cm = cm.scatter_(1, bins, torch.ones(1, dtype=torch.bool).expand_as(bins))
                        freq += (self.countmins[col] * cm).sum(dim=-1)
...
def calculate_intervals(left:int, right:int, intervals:list):
    ...
    # materialize covers as numpy arrays
    # for interval_size in covers:
    #     # print(interval_size, covers[interval_size])
    #     interval_ids = [np.arange(left_id, right_id + 1, dtype=np.int64) for left_id, right_id in covers[interval_size]]
    #     covers[interval_size] = np.concatenate(interval_ids)
    #     # assert len(np.unique(covers[interval_size])) == len(covers[interval_size]), \
    #     #     f'number of unique intervals {len(np.unique(covers[interval_size]))} != number of intervals in cover {len(covers[interval_size])} size {interval_size}'
