...
# Helper function to initialize WORKER with models available for inference of each query
def init_worker(models):
    global WORKER_MODELS
    WORKER_MODELS = models
    # Optionally, you can perform additional per-process initialization here.
    print("Initialized worker with models.")
    
# Helper function for single process
def process_table(id, name, query, models, use_count, cuda):
    global WORKER_MODELS
    
    predicates = query.selects[id]
    keys = query.id2joined_attrs[id]
    components = {attr: query.node2component[f"{id}.{attr}"] for attr in keys}
    print(f'Processing {name}: components = {components}')
    
    t0 = perf_counter_ns()
    estimator, sketch_time = WORKER_MODELS[name](predicates, keys, components=components, count=use_count, cuda=cuda)
    t1 = perf_counter_ns()
    inference_time = t1 - t0 - sketch_time
...
def estimate(query, models, primary=None, cuda=False, method='count-sketch', percentile=0.5):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8, initializer=init_worker, initargs=(models,)) as executor:
        future_to_table = {
            executor.submit(process_table, id, name, query, models, use_count, cuda): (id, name) for id, name in query.table_mapping_iter()
        }
        ...
        for future in concurrent.futures.as_completed(future_to_table):
            ...
            sketching_times.append(res['sketch_time'])
            estimator = res['estimator']
            table_id = res['id']
            
            if method == 'bound-sketch':
                print(f"use_count={use_count} returns {type(estimator)}")
                assert (estimator.sketch_lo >= 0).all(), f"negative values in {res['name']} lo sketch"
                use_count = Fals
        
            sketches_lo[table_id] = estimator.sketch_lo # estimator.sketch_lo.cuda() if cuda else 
    ...
    print("Done with parallel inference...")
